...
pub fn build(b: *std.Build) void {
    ...
    // macOS: link Cocoa framework and compile the ObjC bridge.
    if (target.result.os.tag == .macos) {
        mod.addCSourceFile(.{ .file = b.path("src/systray/macos.m"), .flags = &.{"-fobjc-arc"} });
    ...
    const icon_mod = b.createModule(.{
        .root_source_file = b.path("example/icon.zig"),
        .target = target,
    });
